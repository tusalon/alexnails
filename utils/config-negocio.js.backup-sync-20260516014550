...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = 'dae57f99-8693-4197-be8e-ee2e74f4a337'; // ID de Alex_Nails
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'Alex_Nails';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '53569990';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'alexsandrafabrenapoles@gmail.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a Alex_Nails!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'alexnails';
};
...
console.log('✅ config-negocio.js listo para Alex_Nails');
